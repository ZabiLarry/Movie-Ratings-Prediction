...
def movies_data_reader():
    ...
    basics_data = pd.read_csv(folder + '/basics_data.tsv', sep='\t', header=0, low_memory=False)
    movies_data_ratingless = basics_data[basics_data.titleType == 'movie']
    movies_data_ratingless.columns = ["titleId", "titleType", "primaryTitle", "originalTitle", "isAdult", "startYear",
                                      "endYear", "runtimeMinutes", "genres"]
    useless_cols = ['numVotes', 'titleType', 'originalTitle', 'endYear', 'isAdult']

    akas_data = pd.read_csv(folder + '/akas_data.tsv', sep='\t', header=0, low_memory=False)
    # print(akas_data)
    ...
    ratings_data = pd.read_csv('ratings_data.tsv', sep='\t', header=0, low_memory=False)
    ratings_data.columns = ["titleId", "averageRating", "numVotes"]
    ...
    # old_movies_data.to_csv(folder + '/movies_data_test.csv')
    ...
    movies_data.to_csv(folder + '/clean_movies_data.csv')


def person_reader():
    person_data = pd.read_csv(folder + '/person_data.tsv', sep='\t', header=0, low_memory=False, index_col=0)
    ...
    person_data.to_csv(folder + '/clean_person_data.csv')


def cut_principals():
    person_data = pd.read_csv(folder + '/clean_person_data.csv', header=0, low_memory=False, index_col=0)
    movies_data = pd.read_csv(folder + '/clean_movies_data.csv', header=0, low_memory=False, index_col=1)
    principals_data = pd.read_csv(folder + '/principals_data.tsv', sep='\t', header=0, low_memory=False, index_col=0)
    ...
    cut_principals_data.to_csv(folder + '/clean_principals_data.csv', index_label="titleId")
